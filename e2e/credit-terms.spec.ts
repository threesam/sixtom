...
// the engagement") while /terms bounds it to 30 days, so the disclosure is what
...
const trigger = (page: Page) =>
	page.getByRole('button', { name: /credited in full against the engagement/ })
const panel = (page: Page) => page.getByText(/credited against the engagement if you book one/)

test.describe('teardown credit terms disclosure', () => {
	test.use({ javaScriptEnabled: false })

	for (const route of ['/', '/notify']) {
		test(`${route} discloses the 30-day condition and links to terms, without JS`, async ({
			page
		}) => {
			...
			await expect(panel(page)).toContainText('30 days')
	...
	test('the trigger names itself as a terms disclosure', async ({ page }) => {
		...
		await expect(trigger(page)).toHaveAccessibleName(
			/credited in full against the engagement.*see terms/
		)
	})
})
